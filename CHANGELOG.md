...
- Added **CLK129** detection for UTM parameters placed in the URL path
  (missing `?` or pretty-URL concat). `parse_qsl` and GA4 never see path
  assignments such as `/offer/utm_source=youtube`. Guide paths that mention
  UTM names without `=` are not flagged.
