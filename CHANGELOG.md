...
- Added **CLK120** detection for platform redirect wrappers such as
  YouTube `/redirect`, Facebook `l.php`, Instagram `l.`, Google `/url`,
  and href.li. These links hide the brand destination in a query value, so
  analytics records the wrapper host. Detection is offline and does not
  fetch the nested URL.
