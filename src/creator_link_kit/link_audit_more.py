"""Placement consistency and audit_urls implementation."""
...
from .link_audit_core import (
    _campaign_id_consistency_issues,
    _canonical_link,
    _utm_params,
)
from .link_rules import AuditResult, Issue, validate_url
...
def audit_urls(urls: Iterable[str], convention) -> AuditResult:
    issues: list[Issue] = []
                ...
                if canonical is not None:
                    ...
                    try:
                        ...
                        if parsed.netloc and _authority_error(parsed) is None:
                            ...
                            non_utm = urlencode(
                                sorted((k, v) for k, v in pairs if not k.startswith("utm_"))
                            )
